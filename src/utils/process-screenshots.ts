...
import type { ScreenshotMetadata } from '../api/types/ScreenshotMetadata';
...
export async function processScreenshots(response: AiPromptResponse): Promise<ProcessScreenshotsResponse[]> {
  ...
  const processedScreenshots: ProcessScreenshotsResponse[] = screenshots.map(
    (screenshot: ScreenshotMetadata, index: number) => {
